...
```{r}
install.packages(c("RCurl", "igraph", "ggraph", "ggplot2", "dplyr", "readr", "tibble", "scales"))

# O easylayout é instalado a partir do GitHub:
...
Vamos recuperar nossos resultados de expressão diferencial e preparar a lista de genes que será submetida ao STRING:
...
cat("Total de DEGs para a rede PPI:", length(genes_deg), "\n")
```

## 12. Consultando o STRING via API

O STRING disponibiliza uma [API pública](https://string-db.org/help/api/) que permite realizar consultas programáticas. Vamos utilizá-la para: (1) mapear nossos identificadores de genes para os identificadores internos do STRING, e (2) obter a rede de interações entre eles.
...
```{r}
# Concatenar os genes para a requisição à API
genes_concatenado <- paste0(genes_deg, collapse = "%0d")
...
cat("Genes mapeados no STRING:", nrow(map_ids), "de", length(genes_deg), "\n")
```
...
Vamos aplicar o filtro. Utilizaremos os canais de **co-expressão**, **evidência experimental** e **bancos de dados curados**, com um escore combinado mínimo de **0.4** (média confiança):
...
cat("Interações após filtro (confiança ≥ 0.4):", nrow(int_network_filtered), "\n")
```
...
Para *C. albicans*, um organismo com menos interações experimentalmente validadas, utilizamos o threshold de **média confiança (0.4)** para obter uma rede com número suficiente de arestas. Em organismos-modelo com cobertura extensa, é possível ser mais rigoroso (0.7 ou 0.9).
:::
...
```{r}
# Criar dataframe de anotação mapeando stringId → nomes preferenciais e DEG info
ann <- data.frame(
  stringId = map_ids$stringId,
  gene_name = map_ids$preferredName,
  query = map_ids$queryItem,
  stringsAsFactors = FALSE
...
```{r}
# Criar dicionário gene_name → queryItem (nosso ID Ensembl)
name_to_ensembl <- setNames(ann$query, ann$gene_name)
...
Para este material, utilizamos um layout pré-computado com o algoritmo de Fruchterman-Reingold como alternativa estática:

```{r}
# Alternativa estática (reprodutível sem interação)
set.seed(42)
layout_ppi <- layout_with_fr(g, niter = 1000)
```
...
#| fig-width: 12
#| fig-height: 10
# Preparar dataframe de nós para o plot
node_df <- data.frame(
  name = V(g)$name,
  log2FC = V(g)$log2FC,
  padj = V(g)$padj,
  signif = V(g)$signif,
  neg_log10_padj = -log10(ifelse(V(g)$padj == 0, .Machine$double.xps, V(g)$padj)),
  stringsAsFactors = FALSE
)

ggraph(g, layout = layout_ppi) +
  geom_edge_link(alpha = 0.15, color = "grey60", width = 0.3) +
  geom_node_point(
    aes(color = log2FC, size = neg_log10_padj),
    alpha = 0.85
  ) +
  geom_node_text(
    aes(label = ifelse(neg_log10_padj > quantile(neg_log10_padj, 0.90), name, "")),
    size = 2.8,
    repel = TRUE,
    max.overlaps = 20,
    color = "gray10",
...
::: {.callout-tip}
## Explorando a rede no STRING web

Para uma análise interativa mais detalhada, acesse [string-db.org](https://string-db.org/), selecione **Multiple proteins**, cole a lista de genes DEG, escolha *Candida albicans SC5314 (237561)* como organismo, e explore a rede com as ferramentas visuais do STRING (incluindo clustering automático, enriquecimento funcional integrado e filtros de evidência interativos).
:::

## 17. Métricas de Centralidade

Para identificar os genes mais importantes na rede — aqueles que ocupam posições de **hub** (altamente conectados) ou de **ponte** entre módulos — podemos calcular métricas de centralidade:
...
# Top 15 genes por grau de conectividade
head(node_metrics, 15)
```

### Como interpretar as métricas:

* **Degree (grau)**: número de conexões de cada proteína. Proteínas com alto grau são **hubs** — potenciais reguladores centrais. Se um hub estiver fortemente up ou downregulado, ele pode ser um candidato relevante para investigação experimental.
* **Betweenness (intermediação)**: mede quantos caminhos mais curtos passam por aquela proteína. Proteínas com alta intermediação são **pontes** que conectam diferentes módulos funcionais. Sua perturbação pode ter impacto cascata na rede.

```{r}
#| fig-width: 9
#| fig-height: 6
ggplot(node_metrics, aes(x = degree, y = betweenness, color = signif)) +
  ...
  ggrepel::geom_text_repel(
    data = node_metrics %>% filter(degree >= quantile(degree, 0.92) |
                                    betweenness >= quantile(betweenness, 0.92)),
    aes(label = gene),
    size = 3,
    color = "gray10",
    max.overlaps = 15,
  ...
  labs(
    title = "Centralidade na Rede PPI — Hubs e Pontes",
    x = "Degree (conectividade)",
    y = "Betweenness (intermediação)"
  ) +
  theme_minimal(base_size = 13) +
  theme(
...
::: {.callout-important}
## O que procurar na rede?

No contexto de *Candida albicans* sob pressão de vancomicina:

* **Hubs upregulados (vermelho, alto degree)**: podem indicar genes cuja ativação coordena uma resposta global de adaptação ao estresse antibiótico.
* **Hubs downregulados (azul, alto degree)**: genes centrais cuja repressão pela vancomicina pode comprometer processos essenciais como a biogênese da parede celular ou o metabolismo de ergosterol.
* **Pontes (alto betweenness)**: proteínas que conectam diferentes módulos funcionais. Sua alteração pode sinalizar uma reorganização da comunicação entre processos celulares da *Candida* no microambiente do macrófago.
:::
